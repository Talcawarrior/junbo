...
def is_valid_binary_price(yes_price: float, no_price: float) -> bool:
    ...
    Polymarket prices for near-certain outcomes. Reject 0.0 or 1.0 (Polymarket
    minimum price is $0.0005; 0.0 means no orderbook / invalid data) and
    sums wildly far from 1.0.
    """
    ...
    try:
        ...
        return False
    # Polymarket min price is $0.0005 — 0.0/1.0 mean no real orderbook
    if y <= 0.0 or n <= 0.0 or y >= 1.0 or n >= 1.0:
        return False
    s = y + n
