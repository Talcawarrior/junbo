...
| **SEHIR YA TAM 7 BETLI OLUR YA DA HIC OLMAZ (2026-08-11, KATI KURAL)** | Kullanici karari: bir sehirde forecast center +/- radius (7 esik) icindeki TUM esiklerin acik marketi VAR ve fiyati `0 < yes_price < max_entry` olmali; herhangi biri eksikse O SEHRE HIC girilmez ve o sehrin mevcut acik betleri KAPATILIR. Gerekce: `spread_max_entry=0.50` kurali (0.50+ fiyatli merkez esigi eliyordu) Wellington 11.08'de felaket yaratti — merkez 12C atlandi (fiyat 0.585+ idi) ama 9,10,11,13,14,15 ayaklara girildi -> 6 bet lost, -12.60$. `spread_max_entry` **0.99'a cekildi** (kullanici: "fiyat onemsiz, test icin butun betlere gir"). Kod: `_place_spread_bets_inner` once `targets` (center±3) icin tam-7 kontrolu yapar; eksikse `close_bet_for_rotation` ile mevcut betleri kapatir + sehri skip eder. Test: `test_city_skipped_when_center_market_missing` + `test_city_skipped_when_center_price_high`. Canli uygulama: 11.08'de 56 + 12.08'de 9 eksik bet kapatildi (Austin, Dallas, Denver, Houston, Tokyo, Shenzhen). Suite: 721 -> 723 |
