...
| **Spread sehir secimi "en dogru tahminli" oldu, "en sicak" degil (2026-08-11)** | Kullanici tespiti: backtestler en yuksek olasilikla isabeti, tahmini gercege en yakin tutan (dusuk |bias|) sehirlerde veriyor; 2-3 derece sapan sehirler kayip. Cozum: `spread_placer._city_accuracy(session)` eklenir — `historical_calibrations.bias` uzerinden (sehir -> ortalama |bias|) hesaplar (dusuk = dogru). `_place_spread_bets_inner` sehir secimini `(bias yok mu, ortalama |bias|, -tahmin)` ile siralar: dogru sehirler once, bias'siz yeni sehirler sona. Test: `test_city_accuracy_uses_abs_bias` + `test_spread_prefers_accurate_city_over_hot` (sican ama sapan HOT degil, serin ama dogru ACC secilir). Suite: 712 -> 714 |
