...
| **Bias ground-truth market cozumune aykiri (2026-08-18)** | `scripts/test_metar_vs_settlement.py`: round(METAR max) == Polymarket kazanan bucket **%74 (70/95)** — METAR (NOAA istasyon verisi, WU'nun yayinladigi veri) cozumle uyusuyor. Ama bias hesabinin temeli Open-Meteo Archive actual yalnizca **%30 (55/184)** — bias kalibrasyonu, market cozumunden FARKLI bir dogruluga (gridded ERA5 reanalysis) karsi yapiliyor. Olası spread kayip kaynagi: bias-top sehir secimi yanlis dogrulukla olculuyor. Arastirma: cozum istasyonlarina (settings.py:244-253) karsi bias yeniden kalibre edilebilir. |
...
| **`backtest_metar_peak.py` fake winrate %100 (2026-08-17)** | `gain = (STAKE/entry) - cost if True else -cost` (line 166) gizli dead-code: kazanan bucket Polymarket GERCEK cozumunden alindigi icin her bet WIN sayiliyordu ("winrate %100 / ROI %286" yaniltici ust sinir). Dead-code silindi + docstring'e not eklendi. GERCEKCI sayilar icin `scripts/backtest_metar_peak_realistic.py` (round(actual) ~%30, METAR ~%71 karisim). Test: YOK (backtest script'leri test disi) — onerilen. |
